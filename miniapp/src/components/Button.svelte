...
<script lang="ts">
    import {isDesktop, isDevice, isTgUI} from "../lib/device_utils";
    ...
    let displayText = $derived(isTgUI && text ? text.toUpperCase() : text);
    type = type || 'default';
    if (text) {
        type = 'text';
    }
    const [style, shape] = type.split('-');
    ...
    function onRippleStart(e: MouseEvent | TouchEvent) {
        if ((e instanceof MouseEvent && !isDesktop && style === 'default') || !isTgUI) {
            return;
        }
...
<div bind:this={rippleHandler} class="button {style || 'default'} {shape || ''}" class:isTgUI class:isDesktop role="button" tabindex="0" onclick={click} ontouchstart={onRippleStart} onmousedown={onRippleStart} onkeydown={onKey}>
    {#if isTgUI && style === 'default'}
    ...
    <div>
        {#if type === 'text' && text}
            ...
            {#key text}
                <span class="fade-text" class:isTgUI transition:fade={{ duration: 150 }}>
                    {displayText}
                </span>
            {/key}
        {:else}
...
<style>
    ...
    .button.isTgUI.text::after {
        background: var(--tg-theme-accent-text-color);
    }

    .button:not(.opaque).isDesktop:hover::after,
    .button.default:not(.isDesktop):active::after {
        opacity: 0.05;
    }

    .button:not(.isTgUI):is(.opaque, .text) {
        transition: opacity 120ms ease-out;
    }

    .button:not(.isTgUI):active:is(.opaque, .text) {
        opacity: 0.4;
    }

    .button.default:not(.isTgUI):active > div {
